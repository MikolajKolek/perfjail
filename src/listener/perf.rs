...
pub(crate) struct PerfListener {
    perf_fd: UnsafeCell<Option<OwnedFd>>,
}

impl PerfListener {
    pub(crate) fn new() -> PerfListener {
        PerfListener {
            perf_fd: UnsafeCell::new(None),
        }
    }
}
...
impl Listener for PerfListener {
    ...
    fn on_post_clone_parent(&self, _: &ExecutionContext, parent_data: &mut ParentData) -> io::Result<()> {
        unsafe {
            ...
            let _ = self.perf_fd.as_mut_unchecked().insert(OwnedFd::from_raw_fd(perf_fd));
        }
    ...
    fn on_wakeup(&self, context: &ExecutionContext, parent_data: &mut ParentData) -> io::Result<WakeupAction>{
        if let Some(instruction_count_limit) = context.settings.instruction_count_limit {
            let instructions_used = self.get_instructions_used()?;

            if instructions_used > instruction_count_limit {
                parent_data.execution_result
    ...
    fn on_post_execute(&self, context: &ExecutionContext, parent_data: &mut ParentData) -> io::Result<()> {
        ...
        if let Some(instruction_limit) = context.settings.instruction_count_limit {
        ...
        parent_data.execution_result.set_instructions_used(instructions_used);
        Ok(())
    }
}

impl PerfListener {
    fn get_instructions_used(&self) -> io::Result<i64> {
        let mut instructions_used: i64 = 0;
        
        unsafe {
            let bytes_read = cvt_r(|| {
                read(
                    self.perf_fd.as_ref_unchecked().as_ref().unwrap().as_raw_fd(),
